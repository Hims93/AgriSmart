...
if (isset($_FILES["file"])) {
    ...
    $email = $_POST["email"];
    $mobile = $_POST["mobile"];
    $languge = $_POST["Langauge"];
    $address = $_POST["textarea"];
    $fileName = $_FILES["file"]["name"];
    $tmpname = $_FILES['file']['tmp_name'];
    $folder = "img/" . $fileName;
    $insert = $db->query("INSERT INTO expert_data(name,email,mobile,langauge,queries,file) VALUES ('$name','$email','$mobile','$languge','$address','$fileName')");
    if ($insert) {
